...
return [
    ...
    'info' => [
        ...
        ### Autenticación (app móvil / clientes externos)

        1. `POST /api/login` con `{ "usuario": "...", "contrasena": "..." }`
        2. Usar el `token` devuelto en el header: `Authorization: Bearer {token}`
        3. La mayoría de rutas requieren permisos RBAC además del token.
        4. Usuarios con rol **EMPLEADO** reciben `is_employee_portal: true` e `id_empleado`.

        ### Portal del empleado (móvil-friendly)

        Scoped al `ID_EMPLEADO` del usuario (middleware `portal.employee`):

        - `GET /api/portal/me` — perfil resumido
        - `GET /api/portal/boletas` — boletas de planillas cerradas (paginado)
        - `GET|POST /api/portal/permisos` — mis solicitudes / nueva solicitud
        - `GET /api/portal/encuestas` + `POST .../responder`
        - `GET /api/portal/evaluaciones` (+ `/{id}`)

        ### Cumplimiento SV

        Exportes ISSS, AFP, INSAFORP, F-14/renta, altas-bajas ISSS, aguinaldo y retención 10% bajo `/api/cumplimiento/*`.

        ### OpenAPI

        Descargue la especificación en `/docs/api.json` para generar clientes (Dart, Kotlin, Swift, etc.).
        MD,
    ],

    'ui' => [
        'title' => 'RRHH El Salvador — API',
    ],
     ...
     */
    'servers' => [
        'API' => (static function (): string {
            $url = env('APP_URL');
            $isLocal = in_array(env('APP_ENV', 'production'), ['local', 'development'], true);

            if (! $url) {
                return ($isLocal ? 'http://127.0.0.1:8000' : 'http://localhost') . '/api';
            }

            $url = rtrim($url, '/');

            if ($isLocal && ! parse_url($url, PHP_URL_PORT)) {
                $url .= ':8000';
            }
