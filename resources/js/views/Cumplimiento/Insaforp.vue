  ...
  <DashboardLayout>
    <div class="space-y-6">
      <div class="page-header">
        <div>
          <h1 class="page-title">INSAFORP</h1>
          <p class="page-subtitle mt-1 max-w-2xl">
            Genere el reporte de cotización patronal INSAFORP (1%, techo $1,000 mensual por empleado) a partir de una
            planilla cerrada. Aplica únicamente a empresas con más de 10 empleados.
          </p>
        </div>
      </div>
      ...
      <SkeletonTable v-if="loading" :cols="4" :no-header="true" />

      <template v-else-if="reporte">
        <div v-if="!reporte.aplica_actualmente" class="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 rounded-xl p-3 text-sm">
          <AppIcon name="alert-triangle" size="sm" class="shrink-0 mt-0.5" />
          <span>La empresa actualmente no supera los 10 empleados, por lo que ya no aplica INSAFORP. Este reporte corresponde al histórico de la planilla seleccionada, cuando sí aplicaba.</span>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 col-span-2 sm:col-span-1">
            <div class="text-xs text-slate-500 uppercase font-semibold">Total INSAFORP patronal</div>
            <div class="text-2xl font-bold text-indigo-600 mt-1">${{ fmt(reporte.totales.insaforp) }}</div>
          </div>
        </div>
        ...
        <div class="flex justify-end">
          ...
          >
            ...
            {{ busy ? 'Generando…' : 'Descargar CSV INSAFORP' }}
          </button>
        </div>
          ...
          <table v-table-cards class="table-cards w-full text-sm text-left">
            <thead>
              <tr class="bg-slate-50 dark:bg-slate-700/50 text-xs font-semibold uppercase text-slate-600">
                <th class="px-4 py-3">Código empleado</th>
                <th class="px-4 py-3">Nombre</th>
                <th class="px-4 py-3 text-right">Salario cotizable</th>
                <th class="px-4 py-3 text-right">INSAFORP patronal (1%)</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
              <tr v-for="(f, i) in reporte.filas" :key="i" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                ...
                <td class="px-4 py-3 text-right font-mono">${{ fmt(f.INSAFORP_PATRONAL) }}</td>
              </tr>
              <tr v-if="!reporte.filas.length">
                <td colspan="4" class="px-4 py-10 text-center text-slate-400">
                  Esta planilla no tiene empleados cotizantes de INSAFORP.
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-else class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-10 text-center text-slate-400">
        Seleccione una planilla cerrada para ver el reporte de cotización INSAFORP.
      </div>
    </div>
  </DashboardLayout>
</template>
...
const cargarPreview = async () => {
  ...
  loading.value = true;
  try {
    const { data } = await api.get('/cumplimiento/insaforp/preview', { params: { ID_PLANILLA: planillaId.value } });
    reporte.value = data;
  } catch (err) {
    reporte.value = null;
    toast.error('Error al cargar el reporte INSAFORP', getApiErrorMessage(err));
  } finally {
    loading.value = false;
  }
};
...
const descargar = async () => {
  ...
  busy.value = true;
  try {
    const res = await api.get('/cumplimiento/insaforp/export', {
      params: { ID_PLANILLA: planillaId.value },
      responseType: 'blob',
    });
    const filename = downloadBlobResponse(res, `insaforp_planilla_${planillaId.value}.csv`);
    toast.success('Archivo generado', `${filename} se descargó correctamente.`);
  } catch (err) {
    toast.error('Error al descargar', await getBlobErrorMessage(err, 'No se pudo generar el archivo INSAFORP.'));
  } finally {
    busy.value = false;
  }
};
